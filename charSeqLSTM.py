...
class charSeqLSTM(object):
    ...
    def __init__(self, args):
        ...
        neuralCube_all, targets_all, errWeights_all, numBinsPerTrial_all, cvIdx_all, recordFileSet_all = self.loadAllDatasets()
        ...
        if self.isTraining:
            ...
            for dayIdx in range(self.nDays):
                ...
                if realDataSize>0:
                    realIter = self.makeTrainingDatasetFromRealData(neuralCube_all[dayIdx][trainIdx,:,:], 
...
def combineSynthAndReal(synthIter, realIter):
    ...
    def loadAllDatasets(self, nDays, args, is_training=True):
        neuralCube_all = []
        targets_all = []
        errWeights_all = []
        numBinsPerTrial_all = []
        ...
        for dayIdx in range(nDays):
            neuralData, targets, errWeights, binsPerTrial, cvIdx = prepareDataCubesForLSTM(
                args[f'sentencesFile_{dayIdx}'],
                args[f'singleLettersFile_{dayIdx}'],
                args[f'labelsFile_{dayIdx}'],
                args[f'cvPartitionFile_{dayIdx}'],
                args[f'sessionName_{dayIdx}'],
                args['lstmBinSize'],
                args['timeSteps'],
                is_training
            )
            ...
            synthDir = args[f'syntheticDatasetDir_{dayIdx}']
        ...
        return neuralCube_all, targets_all, errWeights_all, numBinsPerTrial_all, recordFileSet_all                   
    ...
    if __name__ == "__main__":
        ...
        # Aggiungi qui il codice per gestire l'assenza del file, ad esempio creando un file predefinito o terminando il programma.
